...
6. PreCheck is source-read-only, local-first, and cost-bounded. External Evidence producers are disabled by default. Coordinate-only address and bounded nearby-place acquisition may run only after PreCheck freezes the compressed, deduplicated acquisition set and the user authorizes that exact Run request; one decision covers the batch rather than each coordinate. Every Source Item with an available final coordinate receives both component outcomes, while multiple items or components may share Provider work where the adapter can do so honestly. No media, rendition, embedding, path, filename, prompt, or general metadata may cross that boundary. Stage ownership is determined by meaning, not by whether an algorithm happens to be local or remote.
...
8. Policy, confirmation, enforcement, and observed facts establish the declared source-read-only and external-effect boundary. The default path seals a local-only Result with zero external calls. The sole current exception is source-coordinate place acquisition: the Run freezes and deduplicates the exact pending set across all located Source Items, pauses for user confirmation covering address and bounded nearby-place effects, and records authorization plus actual provider effects in the Result.
