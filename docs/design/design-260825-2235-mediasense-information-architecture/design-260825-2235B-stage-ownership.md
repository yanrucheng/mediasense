...
| Evidence | PreCheck for source-derived and Provider-acquired material exposed through the compressed Result. | PreCheck local computation, authorized acquisition, or direct reuse of a Source Item. | Plan reads, expands, transforms into temporary review views, or ignores evidence within its authority; it does not acquire replacement machine evidence. | Plan challenges validity or fitness; PreCheck validates evidence dependencies and effects. | PreCheck Evidence is sealed in the Result. | Existing PreCheck Evidence is immutable after seal. Missing or materially insufficient machine evidence returns to PreCheck; Human semantic input remains distinguishable from evidence. |
