...
1. PreCheck is source-read-only and local-first. Remote calls and data egress are disabled by default. Post-compression coordinate reverse geocoding is permitted only for an exact frozen logical query set confirmed once at the Run/batch boundary and recorded with provider and actual-request evidence; remote models and media, rendition, embedding, path, filename, prompt, or general-metadata egress remain outside this stage. PreCheck may use its own batch engine and shared provider adapters without adopting Plan's per-request Geo Tool workflow.
2. Plan does not replace PreCheck's source facts, candidate evidence, coverage, or declared omissions. It may interpret, ignore, challenge, request replacement, or add a separately owned observation through an authorization-bound stage-neutral capability. Its query purpose, selected coordinates, authorization, state, reuse, and evidence lifecycle are independent from PreCheck's.
...
| Concept | Authoritative owner | Producer | Consumers | Challenger / validator | Seal and formal handoff | Mutation and failure route |
| --- | --- | --- | --- | --- | --- | --- |
| Observation | The stage that acquires and retains it: PreCheck for Result observations; Plan for explicitly authorized Plan-local enrichment. | A stage-owned adapter invokes local extraction, measurement, or a stage-neutral capability. | Plan may use or ignore both classes; Apply does not reinterpret them. | Plan may challenge provenance, availability, or reliability. | PreCheck observations enter the PreCheck Result; Plan-local observations remain revision-bound Working State evidence unless a separate handoff requires them. | A PreCheck observation correction requires a new PreCheck Result. Plan enrichment never edits that Result and advances the Plan revision when retained. |
| Evidence | PreCheck for source-derived material exposed through the compressed result; Plan for separately acquired, Plan-local supporting evidence. | PreCheck local computation or direct reuse of a Source Item; Plan may invoke an authorization-bound stage-neutral capability over exact Result facts. | Plan reads, expands, supplements, transforms into temporary review views, or ignores evidence within its authority. | Plan challenges validity or fitness; each producing stage validates its own dependencies and effects. | PreCheck Evidence is sealed in the Result. Plan-local evidence remains bound to the Working State revision and is shown in review when material. | Existing PreCheck Evidence is immutable after seal. Source decoding or extraction returns to PreCheck; bounded Plan enrichment creates a new Plan observation rather than rewriting upstream evidence. |
...
| Reopen Signal | The detecting stage owns each signal; PreCheck owns resolution of signals targeting source accounting, decoding, extraction, or Result evidence preparation. | PreCheck may record known triggers before seal; Plan may issue a new signal during planning. | PreCheck consumes Plan-issued signals; Plan consumes known triggers in the result. | The target stage validates whether the trigger is material and records its resolution. | Known triggers are in the PreCheck Result. A Plan-issued signal is a routing record, not a fourth formal handoff. | Immutable once issued. It is resolved by continuing PreCheck work and sealing a new result. A bounded Plan-local Geo observation does not require reopening PreCheck and never edits the old Result. |
...
- the complete Frozen Plan object returned by Plan seal, including schema,
  supported encoding profile, identity, freeze state, and exact confirmation;
- the compatible PreCheck Result and Dataset binding declared by the plan;
- complete Source Set expansion through the public PreCheck Read boundary, without
  trusting a caller completeness flag or reading PreCheck private storage;
...
Apply does not discover a Plan-private artifact path and performs no geographic
lookup or place interpretation. Current source-root and destination bindings remain
Apply inputs because they authorize real filesystem locations, not Plan semantics.
