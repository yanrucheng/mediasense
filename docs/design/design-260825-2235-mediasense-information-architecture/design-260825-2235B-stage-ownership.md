...
1. PreCheck is source-read-only and local-first. Remote calls and data egress are disabled by default. Source-coordinate reverse geocoding is permitted only for an exact frozen logical query set confirmed once at the Run/batch boundary and recorded with provider and actual-request evidence; every Source Item with an available final coordinate receives an outcome, regardless of visual compression. Remote models and media, rendition, embedding, path, filename, prompt, or general-metadata egress remain outside this stage. PreCheck may use its own batch engine and shared provider adapters without adopting Plan's per-request Geo Tool workflow.
