...
2026-09-17 独立验收补记：B 的来源、有限读取、逐项持久化、授权入口和原性能证据已获确认，但整体验收因 Host 取消收尾及跨 Host 锁竞争误判暂不通过。随后在既有 Apply Run/worker 内补修调度、取消意图保持与正常锁竞争分类，新增 9 项真实 Host 控制测试；新的构建身份及验证结果记录于 [Host 控制补修](../../../eval/sessions/260917-1357-apply-b-validation/report.md#host-控制补修2026-09-17待用户独立复验)。不覆盖首轮证据，不宣称已完成用户独立复验；仍未发布、安装或执行真实媒体。


进一步复核发现跨 Host status 可在正常启动窗口误判 owner 消失。后续补修增加 Run 内部的共享存活锁，与既有排他执行锁共同协调接受控制至 worker 退出的连续持有；固定双 Host/独立进程反例与真实退出、启动失败测试见 [启动窗口补修记录](../../../eval/sessions/260917-1357-apply-b-validation/report.md#跨-host-启动窗口补修2026-09-17待独立复验)。B 的媒体 I/O、提交、公开 JSON 和迁移处置不变；新构建仍待独立复验，未发布安装或操作真实媒体。
