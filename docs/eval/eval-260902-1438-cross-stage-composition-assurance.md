...
| PreCheck Read -> Plan Geo subject validation | PreCheck Read, Plan Work, and Geo contracts | `PlanWorkTool` / `PlanGeoAdapter` | Shared port plus focused Geo and Plan integration tests | Proven at component boundary |
...
| Geo Tool -> Plan retained observation | Geo and Plan Work contracts | `PlanGeoAdapter` | Real capability semantics with fake providers and focused Plan integration | Component-proven; composition dispatch only partially exercised |
