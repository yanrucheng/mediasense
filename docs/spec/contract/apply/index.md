...
One localized item failure does not change `executing` to `failed`; independent safe work continues, then the Run enters `needs_attention` if unresolved items remain. A process crash is not a public state. A Host with no local worker must establish across processes that neither a live control call or worker handoff nor an active execution owns the Run before exposing `needs_attention` with an interruption reason. A temporarily free execution lock alone is not proof of interruption. Ownership observation covers control acceptance through worker takeover and exit; process death releases the corresponding liveness protection. Status never starts media effects. Explicit resume reconciles durable intent with source and destination facts. Unexpected worker exceptions surface and record `failed` with possible effects, rather than becoming an ordinary wait.
