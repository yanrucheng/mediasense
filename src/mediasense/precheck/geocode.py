    ...
    def produce(
        ...
        *,
        ...
        if not profile.enabled:
            return ReverseGeocodeBatchOutcome("policy_disabled", batch, (), 0)
        if batch.pending_query_count:
            ...
            if decision == "skip_optional_work":
                return ReverseGeocodeBatchOutcome("not_requested", batch, (), 0)
            if decision != "proceed":
        ...
        if self.geocoder is None and batch.pending_query_count:
            raise RuntimeError("no reverse-geocode provider is configured")
