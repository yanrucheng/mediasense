...
本轮人工认可的 DINOv3 ViT-B/16 **384px** 配方现已接入正式 CLI/MCP 的本地 embedding
选择，并完成本机 Core ML 安装与小规模真实推理。启用仍需显式配置；旧 ChineseCLIP
配置不自动换模型。安装、平台范围和固定依赖以[安装 runbook](installation.md#dinov3-384-local-embedding)
为准，发布身份、验证和未决粒度选择见既有迁移台账。
下文“未替换生产模型／未确定生产默认粒度”等表述保留评测当时的决定范围，
不扩大为本次已确认所有分组参数，也不改写原评测配置、向量或性能。
