...
with a fresh `request_id`, the Plan artifact path, `effect: move_originals`, the
current root for every referenced `source_root_ref`, and the destination parent.
Preparation must remain zero-media-effect.
