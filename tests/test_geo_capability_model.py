...
def test_request_fingerprint_is_independent_of_subject_order() -> None:
    first = GeoRequest(
        GeoOperation.REVERSE_GEOCODE,
        (
            GeoSubject("source-item:a", GeoCoordinate(22.3, 114.1)),
            GeoSubject("source-item:b", GeoCoordinate(22.4, 114.2)),
        ),
        "zh-CN",
    )
    reordered = GeoRequest(
        GeoOperation.REVERSE_GEOCODE,
        tuple(reversed(first.subjects)),
        "zh-CN",
    )
...
def test_authorization_requires_exact_identity_and_timezone() -> None:
    ...
    with pytest.raises(ValueError, match="sha256 identity"):
        GeoAuthorization(
            "human:one", "request:one", datetime.now(timezone.utc), envelope
        )
    ...
    def execute(
        ...
        *,
        ...
        candidates = (
            ...
            if self.status is GeoComponentStatus.SUCCESS
            else ()
        )
        return GeoProviderExecution(
            GeoComponentResult(
                operation, self.status, ("provider",), coordinate, candidates
            ),
            GeoProviderAttempt(
                self.capabilities.provider_id,
                operation,
...
def test_capability_executes_with_exact_authority_and_offers_bounded_continuation() -> (
    None
):
    from mediasense.capabilities.geo import GeoCapability
...
def test_provider_request_ceiling_is_enforced_across_the_whole_batch() -> None:
    from mediasense.capabilities.geo import GeoCapability
    ...
    assert result.components[1].qualifications[0]["code"] == (
...
def test_batch_order_does_not_change_each_coordinate_route_seed() -> None:
    from mediasense.capabilities.geo import GeoCapability
