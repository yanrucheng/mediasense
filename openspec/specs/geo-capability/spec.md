...
ceiling when knowable, and retention scope. Without matching authority it SHALL
perform no network effect and return the exact proposed authorization envelope.
...
- **THEN** the Tool refuses to begin that additional effect until authority is supplied for the exact delta
...
- **THEN** the Tool returns `refused` and sends zero provider requests
...
`no_result`, `refused`, `unavailable`, `failed`, and `indeterminate` outcomes SHALL
...
Geo routing state SHALL be immutable and request-scoped. The capability SHALL NOT
read or mutate PreCheck or Plan private storage, confirmation state, Work records,
or caches.

#### Scenario: PreCheck and Plan calls overlap
- **WHEN** PreCheck and Plan perform concurrent Geo operations
