...
**这是方案 B 的开发设计草案，当前只允许规划。** 开始条件见 [README](README.md)；本页的字段和配置示例尚未成为公开合约。

用户已选择 Freepik/nsfw_image_detector 与 NudeNet 640m，并确认“图片和执行要求输入，具名属性与执行情况输出”的方向。模型内部方法由适配器负责；PreCheck 取得本地证据，Plan 判断如何使用。用户授权 Agent 自行确定有充分依据的工程细节，但明确要求暂不开发。
...
**规划默认 D5-disabled（尚非用户单独确认）：** 停用后，新 Result 不纳入该模型的缓存输出；旧 Result 和缓存保留。重新启用时仍可复用有效 Work。不增加 cache-only 模式。若用户在开发放行前选择“停用仅停止新推理、仍交付缓存”，必须同步修订本节、停用样例及对应 delta/验收，再进入开发。
...
| 停用含义未单独获得答复 | 采用已标明的规划默认；开始开发前展示并确认适用意图，用户选择改变时只修受影响部分 |

## Migration Plan

1. **当前停在这里：** 完成包内文档和合成样例校验，告知用户计划就绪；未收到后续开始指示不得执行 tasks 的开发部分。
...
唯一保留的产品取舍是 D5-disabled：本包按“停用不参与新结果，包括缓存”编制，用户尚未对这条异步问题单独作答。用户已要求先编好计划，所以该点不阻止规划，也不被记录为已经确认。

其余工程选择有明确默认和验证责任。精确 Python 类型名、文件拆分、资源估计的保守余量、内部存储列及发布版本号由实施者依据届时基线决定。只有实际证据要求改变已选配方、信息含义、效果边界或数据保留保证时，才重新提出产品讨论。
